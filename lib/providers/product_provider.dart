...
@riverpod
Future<ProductDetail> productDetail(ProductDetailRef ref, int id) async {
  final service = ref.watch(productServiceProvider);
  return service.getProductDetail(id);
}

@riverpod
Future<AddProduct> addProduct(AddProductRef ref, String title) async {
  final service = ref.watch(productServiceProvider);
  return service.addProduct(title);
}

@riverpod
Future<UpdateProduct> updateProduct(UpdateProductRef ref, UpdateParams params) async {
  final service = ref.watch(productServiceProvider);
  return service.updateProduct(params.title, params.id);
}

@riverpod
Future<DeleteProduct> deleteProduct(DeleteProductRef ref, int id) async {
  final service = ref.watch(productServiceProvider);
